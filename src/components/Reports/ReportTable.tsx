...
const ReportTable: React.FC<ReportTableProps> = ({ data }) => {
  ...
  const filterBySearch = (item: SearchableItem) => {
    if (!data.filters.searchQuery) return true;
    const searchLower = data.filters.searchQuery.toLowerCase();
    
    // Handle different property names for different item types
    const name = item.name || item.patientName || '';
    const identifier = item.mrn || item.medicalNumber || '';
  ...
  const filteredConsultations = data.consultations.filter(consultation => 
    filterDataByDate(consultation) && filterBySpecialty(consultation) && filterBySearch({
